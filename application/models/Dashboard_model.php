...
class Dashboard_model extends CI_Model
{
    ...
    // Fixed where_not_in array issue 
        ...
        try {
            for ($month = 1; $month <= 12; $month++) {
                ...
                foreach ($result as $row) {
                    ...
                    $sql2 = "SELECT SUM(`h`.`amount`) AS `total_paid`, SUM(`h`.`discount`) AS `total_discount` FROM `fee_payment_history` as `h` WHERE `h`.`allocation_id` = " . $this->db->escape($row->allocation_id) . " AND  `h`.`type_id` = " . $this->db->escape($row->fee_type_id);
                    $r = $this->db->query($sql2)->row();
                    if ($r) {
                        $totalpaid += floatval($r->total_paid);
                        $total_discount += floatval($r->total_discount);
                    }
                }
            ...
            log_message('error', 'Annual fees summary error: ' . $e->getMessage());
    ...
    public function get_total_student($branchID = '')
    {
        try {
            ...
            return $result ? $result->total_student : 0;
        } catch (Exception $e) {
            log_message('error', 'Get total student error: ' . $e->getMessage());
            return 0;
        }
    }
